...
async function calcTotalTransferred() {
	...
	const sums = await db.query(`
		SELECT bridge_id, type, home_network, foreign_network, home_asset, home_symbol, SUM((amount+0.0) / SUBSTR('10000000000000000000000000000000', 1, (CASE WHEN type='expatriation' THEN home_asset_decimals ELSE foreign_asset_decimals END)+1)) AS total
		FROM transfers
		LEFT JOIN bridges USING(bridge_id)
		GROUP BY transfers.bridge_id, type`
	);
	for (let { bridge_id, type, home_network, foreign_network, home_asset, home_symbol, total } of sums) {
		const rate = await fetchExchangeRateInUSD(home_network, home_asset, true);
