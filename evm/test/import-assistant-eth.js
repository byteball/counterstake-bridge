...
contract("Importing GBYTE with ETH staking and assistance", async accounts => {
	...
	it("bob buys shares in the assistant", async () => {
		...
		let res = await assistant.buyShares(eth_amount, gbyte_amount, 0, { value: eth_amount, from: bobAccount });
	...
	//	const aliceAccount = token.address
		...
		const sender_address = "SENDER"
		const data = ""
		const reward = amount.div(new BN(5))
	...
	*/
	...
	//	this.claim.withdrawn = false; // stays false
		...
		this.recent_stake_profit = this.recent_stake_profit.mul(new BN(10 * 24 * 3600 - elapsed)).div(new BN(10 * 24 * 3600)).add(ether('0.2'))
	...
	it("bob redeems some shares", async () => {
		...
		let stake_payout = stake_net_balance.sub(unavailable_stake_profit).mul(shares_amount).div(ether('7'))
		let image_payout = image_net_balance.sub(unavailable_image_profit).mul(shares_amount).div(ether('7'))
	...
	it("alice buys some shares", async () => {
		...
		const res = await assistant.buyShares(eth_amount, gbyte_amount, 0, { value: eth_amount, from: aliceAccount });
		const ts = (await web3.eth.getBlock(res.receipt.blockNumber)).timestamp;
	...
	it("manager withdraws his management fee", async () => {
		...
		expect(manager_stake_balance_before.sub(gasCost).add(full_withdrawn_stake)).to.be.bignumber.eq(manager_stake_balance_after)
	...
	it("claim 4", async () => {
		...
		const sender_address = "SENDER"
		const data = ""
		const reward = amount.mul(new BN(60)).div(new BN(100))
		const paid_amount = amount.sub(reward)
		this.paid_amount = paid_amount
		let res = await assistant.claim(txid, txts, amount, reward, sender_address, aliceAccount, data, { from: managerAccount });
