...
describe('Creating export transaction', function () {
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		for (let name in vars) {
			if (name.startsWith('export_')) {
				this.export_aa = name.substr('export_'.length)
				expect(vars[name]).to.deep.equalInAnyOrder({
					asset: this.asset,
					asset_decimals: 9,
					foreign_asset: this.foreign_asset,
					foreign_network: this.foreign_network,
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(response.response.responseVars['amount']).to.be.equal(1e9)
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(vars['1_yes_by_' + this.bobAddress]).to.be.eq(this.amount)
	...
	it('Bob waits and withdraws without being challenged', async () => {
	...
	it("Bob challenges her claim but his amount is not sufficient to overturn the outcome", async () => {
		...
		this.claim2.stakes.no += this.amount2 * 1.1
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(vars['o_' + this.claim_num2]).to.deep.equalInAnyOrder(this.claim2)
		expect(vars['2_yes_by_' + this.aliceAddress]).to.be.eq(this.amount2)
		expect(vars['2_no_by_' + this.bobAddress]).to.be.eq(this.amount2 * 1.1)
	})

	it("Alice supports Bob's challenge and overturns the outcome", async () => {
		const { unit, error } = await this.alice.triggerAaWithData({
			toAddress: this.export_aa,
			amount: this.amount2 * 0.4 + 2000,
			data: {
				claim_num: this.claim_num2,
				stake_on: 'no',
			},
		...
		const ts = Math.round((await this.alice.getTime()).time/1000)
		this.claim2.stakes.no += this.amount2 * 0.4
		this.claim2.current_outcome = 'no'
		this.claim2.period_number = 1
		this.claim2.expiry_ts = ts + this.challenging_periods[1] * 3600
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		expect(vars[this.claim_num2 + '_no_by_' + this.bobAddress]).to.be.eq(this.amount2 * 1.1)
		expect(vars[this.claim_num2 + '_no_by_' + this.aliceAddress]).to.be.eq(this.amount2 * 0.4)
	})
	...
	//	await this.network.witnessUntilStable(response.response_unit)
		...
		console.log('vars', vars)
		expect(vars[this.claim_num2 + '_yes_by_' + this.aliceAddress]).to.be.eq(this.amount2)
		expect(vars[this.claim_num2 + '_no_by_' + this.bobAddress]).to.be.undefined
		expect(vars[this.claim_num2 + '_no_by_' + this.aliceAddress]).to.be.eq(this.amount2 * 0.4)
		...
		expect(Utils.getExternalPayments(unitObj)).to.deep.equalInAnyOrder([
			{
				address: this.bobAddress,
				amount: Math.floor(1.1/1.5 * 2.5 * this.amount2),
			},
		])
		const data = unitObj.messages.find(m => m.app === 'data').payload
		expect(data).to.be.deep.eq({
	...
	it('Alice withdraws too', async () => {
		...
		expect(vars[this.claim_num2 + '_yes_by_' + this.aliceAddress]).to.be.eq(this.amount2)
		expect(vars[this.claim_num2 + '_no_by_' + this.bobAddress]).to.be.undefined
		expect(vars[this.claim_num2 + '_no_by_' + this.aliceAddress]).to.be.undefined
		expect(vars['o_' + this.claim_num2]).to.be.undefined
		expect(vars['f_' + this.claim_num2]).to.deep.equalInAnyOrder(this.claim2)
		...
		expect(Utils.getExternalPayments(unitObj)).to.deep.equalInAnyOrder([
			{
				address: this.aliceAddress,
				amount: Math.floor(0.4/1.5 * 2.5 * this.amount2),
			},
		])
		const data = unitObj.messages.find(m => m.app === 'data').payload
		expect(data).to.be.deep.eq({
		...
		const new_aa_balance = (await this.alice.getOutputsBalanceOf(this.export_aa)).base.total
