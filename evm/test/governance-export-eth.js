...
contract("Governance for exporting ETH", async accounts => {
	...
	it("alice suggests setting challenging periods that get shorter and fails", async () => {
		let promise = challengingPeriodsVotedValue.voteAndDeposit([new BN(13 * 3600), new BN(12 * 3600)], ether('10'), { value: ether('10'), from: aliceAccount });
	...
	it("alice suggests setting too long challenging periods and fails", async () => {
		let promise = largeChallengingPeriodsVotedValue.voteAndDeposit([new BN(12 * 3600), new BN(3 * 365 * 24 * 3600 + 1)], ether('10'), { value: ether('10'), from: aliceAccount });
