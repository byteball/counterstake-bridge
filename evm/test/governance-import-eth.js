...
contract("Governance for importing with ETH stakes", async accounts => {
	...
	before(async () => {
		...
		token = await Token.new("Some token", "TKN");
	...
	// 	let promise = largeThresholdVotedValue.voteAndDeposit(bn0, ether('10'), { value: ether('10'), from: aliceAccount });
	...
	it("start a claim", async () => {
		...
		const txts = Math.floor(Date.now()/1000)
		const amount = ether('50')
		const stake = ether('1.2') // amount * 1.2
		const sender_address = "SENDER"
		const data = ""
		const reward = bn0
	...
	it("start a claim", async () => {
		...
		const txts = Math.floor(Date.now()/1000)
		const amount = ether('50')
		const stake = ether('2.4') // amount * 1.2 * 1/25
		const sender_address = "SENDER"
		const data = ""
		const reward = bn0
		let res = await instance.claim(txid, txts, amount, reward, stake, sender_address, a0, data, { value: stake, from: aliceAccount });
